...
Ubuntu Desktop: current Ubuntu release is 26.04, but the installation procedure works on earlier versions (maybe some slight modifications) 
...
- for the Macintosh computers with an **Apple Silicon processor** (M1/M2/M3), you absolutely need to download the **"arm" version** of the Ubuntu installation disk image. `
ubuntu-26.04-desktop-arm64.iso`

- for other computers, the **"amd" install disk** is fine: `ubuntu-26.04-desktop-amd64.iso`
...
- Disk Size: 25Gb, dynamically allocated
...
- File location and size
    ...
    - max size of virtual hard drive:	    80GB

        **Note:** the size has to be larger than your real needs, because Ubuntu will automatically create some big  partitions (/dev and /run/shm). Since we allow dynamical memory allocation, it is fine to set the max size to 80Gb, which will enable you to install the operating systems, the languages, libraries and tools, and to dispose of some space for installing genomes.
-->
...
1. In the **File** menu, select *Tools > Network*
...
By default, we open a network adaptor of type **NAT**. This allows to establish connections 
- from your VM (browser, ssh) to the world outside the host machine (your computer)
- from your computer to the VM
...
### Port forwarding for the ssh connection (terminal)
...
### Port forwarding for the web browser (Apache)
...
You can also add a port for https  (secured Web access) 
- Name: https
- Protocol: TCP
- Host port: 8443
- Guest port: 443
...
**Note: I suppress this (obsolete in Ubuntu 24.04.3), but I did not find a solution to auto-resize screen with MacARM distribution
